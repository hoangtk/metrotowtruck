            ...
            if invoice.move_id:
                for m in invoice.move_id.line_id:
                    ...
                    if m.reconcile_id:
                        ...
                        temp_lines = map(lambda x: not x.purchase_ids and not x.sale_ids and x.journal_id in('cash','bank') and x.id, m.reconcile_id.line_id)
                    elif m.reconcile_partial_id:
                        #only include the account move with cash/bank journal and not related with so/po
                        temp_lines = map(lambda x: not x.purchase_ids and not x.sale_ids and x.journal_id in('cash','bank') and x.id, m.reconcile_partial_id.line_partial_ids)
