...
def trans_export(lang, modules, buffer, format, cr):
    ...
    def _process(format, modules, rows, buffer, lang):
        if format == 'csv':
            ...
            '''
            '''
            johnw, 12/31/2014, use the source/translation as group key, fix the issue: when export .po file, 
            for the same key, all of the items exported as one key, this will cause issue when the translation is difference among one same key, only the first translation will be exported.
            we now group the translations by source and translate value. That means one translation per source/translation.
            '''
            grouped_rows = {}
            for module, type, name, res_id, src, trad, comments in rows:
                group_key = '_trans_src:%s_trans_val:%s'%(src,trad)
                row = grouped_rows.setdefault(group_key, {})
                row.setdefault('modules', set()).add(module)
                if not row.get('translation') and trad != src:
                    row['translation'] = trad
                row.setdefault('tnrs', []).append((type, name, res_id))
            ...
            for src, row in grouped_rows.items():
                idx_start = src.find('_trans_src:') + len('_trans_src:')
                idx_end = src.find('_trans_val:')
                src = src[idx_start:idx_end]
                if not lang:
                    # translation template, so no translation value
                    row['translation'] = ''
                elif not row.get('translation'):
...
def trans_load_data(cr, fileobj, fileformat, lang, lang_name=None, verbose=True, module_name=None, context=None):
    ...
    try:
        ...
        print '=========file name:%s'%fileobj.name
        for row in reader:
            line += 1
