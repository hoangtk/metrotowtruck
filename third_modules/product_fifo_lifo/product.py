    ...
    def _compute_qty_obj(self, cr, uid, from_unit, qty, to_unit, round=True, context=None):
        ...
        if from_unit.category_id.id <> to_unit.category_id.id:
            if context.get('raise-exception', True):
                raise osv.except_osv(_('Error!'), _('Conversion from Product UoM %s(ID:%s,Category:%s) to Default UoM %s(ID:%s,Category:%s) is not possible as they both belong to different Category!.') % (from_unit.name,from_unit.id,from_unit.category_id.name,to_unit.name,to_unit.id,to_unit.category_id.name,))
            else:
                return qty
