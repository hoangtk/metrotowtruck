...
class future_ship_req_line(osv.osv_memory):
    ...
    _columns = {
        ...
        'future_shipment_req_id' : fields.many2one('future.ship.requi', 'Shipment'),
    ...
    def _check_product_qty(self, cursor, user, ids, context=None):
        for line in self.browse(cursor, user, ids, context=context):
            _constraints = [(_check_product_qty,'Product quantity: ',['product_qty'])]#_constraints 还有browse no understan
...
#    _description = 'Requisition\'s Future shipping items'
    _columns = {
                'line_ids' : fields.one2many('future.ship.req.line', 'wizard_id', 'Products'),
        
#        'shipment_id': fields.one2many('future.ship.req.line','product_id', 'Products002', required=True),
    }
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        req_obj = self.pool.get('future.shipment.line') # self.pool.get??
        req = req_obj.browse(cr, uid, record_id, context=context)#   AttributeError: "Field 'line_ids' does not exist in object 'browse_record(future.shipment, 7)'"
        partner_id = None
        for line in req.line_ids:
            if not line.generated_po:
                ...
                if partner_id == None and line.product_id.seller_id.active:
                        ...
                        res.update({'shipment_id': line_data,})
        return res
...
#class future_shipment_req(osv.osv):
#    '''
#    doc
#    '''
#    
#    _name = "future.shipment.req"
#    _columns = {
#                'product_waiting':fields.many2one('product_id','Remaining Items'),
#                'product_shipped':fields.many2one('product_id','Shipped Items'),
#                'product_qty': fields.float('Quantity', digits_compute=dp.get_precision('Product Unit of Measure'),required=True),
#                'product_qty_remain': fields.float('Quantity Remain', digits_compute=dp.get_precision('Product Unit of Measure'),required=True),
